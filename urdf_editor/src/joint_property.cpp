...
namespace urdf_editor
{
  JointProperty::JointProperty(urdf::JointSharedPtr joint, QStringList &link_names, QStringList &joint_names): joint_(joint), manager_(new QtVariantPropertyManager()), factory_(new QtVariantEditorFactory()), link_names_(link_names), joint_names_(joint_names)
  {
   ...
   // add the link list as an enum to the parent drop down menu. pass the first item into the joint data struct, if there is one
    ...
    if(link_names_.size() > 0 && joint_->parent_link_name=="") {
      joint_->parent_link_name = link_names_[0].toStdString();
    }
    ...
    if(link_names_.size() > 0 && joint_->child_link_name=="") {
      joint_->child_link_name = link_names_[0].toStdString();
    }
  ...
  QString JointProperty::getChildLinkName()
  {
    return QString::fromStdString(joint_->child_link_name);
  }

  /*! Get the parent link name */
  QString JointProperty::getParentLinkName()
  {
